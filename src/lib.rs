...
//! Welcome to the `rasn` (pronounced "raisin"), a safe `#[no_std]` ASN.1 codec
...
//! Welcome to ASN.1 and DER"*][lenc] by Let's Encrypt as a quick introduction
...
//! ASN.1. Now with `rasn`'s abstract model you can build and share ASN.1 data
...
//!
//! ### `#[no_std]` Support
//! Rasn is entirely `#[no_std]`, so you can handle and share the same ASN.1
//! code with a wide variety of platforms and devices.
//!
//! ### Rich Data Types
//! Rasn currently has support for nearly all of ASN.1's data types. `rasn`
//! uses popular community libraries such as `bitvec`, `bytes`, and `chrono`
//! for some of its data types as well as providing a couple of its own. Check
//! out the [`types`](./types) module for what's currently available.
//!
//! ### Safe BER, CER, and DER Codecs
//! Included with the framework is a implementation of the X.690 standard also
//! known as the Basic Encoding Rules, Canonical Encoding Rules, and
//! Distinguished Encoding Rules codecs. The encoder and decoder have been
//! written in 100% safe Rust and fuzzed with [American Fuzzy Lop][bun] to
//! ensure that the decoder correctly handles random input, and if valid that
//! the encoder can correctly re-encode that value.
//!
//! [bun]: https://lcamtuf.coredump.cx/afl/
//!
//! ### Powerful Derive Macros
//! Easily model your structs and enums with derive equivalents of all of the
//! traits. These macros provide a automatic implementation that ensures your
//! model is a valid ASN.1 type at *compile-time*. To explain that though,
...
//! ensuring it's correctly encoded and decoded. In `rasn` this is completely
//! abstracted away letting you focus on the abstract model. Let's look at what
//! decoding a simple custom `SEQUENCE` type looks like.
//!
//! ```asn1
//! Person ::= SEQUENCE {
//!   age INTEGER,
...
//! implement. `Decode` and `Encode` for converting to and from encoding rules,
//! and the shared `AsnType` trait; which defines some associated data needed
...
//! ```
//!
//! Next is the `Decode` and `Encode` traits. These are mirrors of each other
//! and both have one provided method (`decode`/`encode`) and one required
//! method (`decode_with_tag`/`encode_with_tag`). Since in ASN.1 nearly every
//! type can be implicitly tagged allowing anyone to override the tag
//! associated with the type, having `*_with_tag` as a required method requires
//! the implementer to correctly handle this case, and the provided methods
//! simply calls `*_with_tag` with the type's associated `AsnType::TAG`. Let's
//! look at what the codec implementation of `Person` looks like.
//!
//! ```rust
//! use rasn::{Decode, Decoder, Encode, Encoder, Tag, types::Integer};
//!
...
//! ```
//!
//! That's it!  We've just created a new ASN.1 that be encoded and decoded to
//! BER, CER, and DER; and nowhere did we have to check the tag, the length, or
//! whether the string was primitive or constructed encoded. All those nasty
//! encoding rules details are completely abstracted away so your type only has
//! handle how to map to and from ASN.1's data model. With all the actual
//! conversion code isolated to the codec implementations you can know that
//! your model is always safe to use.
//!
...
//! Encode` implementation, and what if we forgot to use the encoder we were
//! given in `encode_sequence` and tired to use the parent instead?
//!
//! ```no_compile
...
//! semantics also mean that an `Encoder` can't accidentally encode the
...
//! types can be identified by a tag (essentially two numbers e.g. `INTEGER`'s
//! tag is `0, 2`). Field and variant names are not transmitted in most
//! encoding rules, so this tag is also used to identify fields or variants in
//! a `SEQUENCE` or `CHOICE`. This means that every that in a ASN.1 struct or
//! enum every field and variant  **must have** a distinct tag for the whole
//! type to be considered valid. For example ; If we changed `age` in `Person`
//! to be a `String` like below it would be invalid ASN.1 even though it
//! compiles and runs correctly, we have to either use a different type or
//! override `age`'s tag to be distinct from `name`'s. When implementing the
...
//! ASN.1 model implementation implemented declaratively. The `Encode` and
//! Decode` macros will essentially auto-generate the implementations we showed
//! earlier, but the real magic is the `AsnType` derive macro. Thanks to the
//! static-assertations` crate and recent developments in `const fn`; the
//! `AsnType` derive will not only generate your `AsnType` implementation, it
...
//! `#[rasn(tag)]` attribute to override the tags of one or more of the types.
//! However we can actually go further, because in ASN.1 there's the concept of
//! having `AUTOMATIC TAGS` which essentially tells your ASN.1 compiler to
//! automatically generate distinct tags for your ASN.1 definition. Now with
//! rasn you can do that in Rust! Applying `#[rasn(automatic_tags)]` to the
